...
import { scrollProductsGridDown } from '../covetrus/covetrus-equipment-grid-scroll';
...
const IDLE_SCROLLS_BEFORE_STOP = 6;
...
export class CovetrusScrapeConsumer extends WorkerHost {
  ...
  private async processScrapeCategory(
    ...
    const expectedTotal = job.data.categoryCount;
    ...
    await this.covetrusSession.withCatalogPage(async (page) => {
      ...
      try {
        ...
        const targetMetEarly =
          expectedTotal != null && seenSku.size >= expectedTotal;
        ...
        if (!targetMetEarly) {
          ...
          for (let step = 0; step < MAX_SCROLL_STEPS; step += 1) {
            ...
            if (batch.length > 0) {
              ...
              idleScrolls += 1;
              if (idleScrolls >= IDLE_SCROLLS_BEFORE_STOP) {
                break;
              }
            }
